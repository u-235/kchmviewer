		...
		else if ( state == STATE_IN_HTML_ENTITY )
		{
			...
			if ( ch != ';' && ch != '<' )
			{
				if ( parseentity.isEmpty() )
				{
					...
					qWarning( "Index::parseDocument: incorrectly terminated HTML entity '&%s%c', ignoring", qPrintable( parseentity ),
							  ch.toLatin1() );
...
QList< QUrl > Index::query( const QStringList& terms, const QStringList& termSeq, const QStringList& seqWords,
							EBook* chmFile )
{
	QList<Term> termList;
...
bool Index::searchForPhrases( const QStringList& phrases, const QStringList& words, const QUrl& filename,
							  EBook* chmFile )
{
